...
public class Test08ListarMercaderiasPorCategoriaProfe {
    ...
    @Test
    void listadoIngresoFiltrando() {
        s.registrarMercaderia("COD03", "XX-001-XXX122", "Descripción 3", false, Categoria.OTROS);
        s.registrarMercaderia("COD01", "XX-001-XXX123", "Descripción 1", false, Categoria.ALIMENTOS);
        s.registrarMercaderia("COD02", "XX-001-XXX121", "Descripción 2", false, Categoria.OTROS);
        retorno = s.listarMercaderiasPorCategoria(Categoria.OTROS);
        assertEquals(Retorno.Resultado.OK, retorno.getResultado());
        assertEquals("COD02;XX-001-XXX121;Descripción 2;false;Otros|COD03;XX-001-XXX122;Descripción 3;false;Otros", retorno.getValorString());

        retorno = s.listarMercaderiasPorCategoria(Categoria.ALIMENTOS);
        assertEquals(Retorno.Resultado.OK, retorno.getResultado());
        assertEquals("COD01;XX-001-XXX123;Descripción 1;false;Alimentos", retorno.getValorString());

        retorno = s.listarMercaderiasPorCategoria(Categoria.ELECTRONICA);
        assertEquals(Retorno.Resultado.OK, retorno.getResultado());
        assertEquals("", retorno.getValorString());

        retorno = s.listarMercaderiasPorCategoria(Categoria.DOCUMENTACION);
        assertEquals(Retorno.Resultado.OK, retorno.getResultado());
        assertEquals("", retorno.getValorString());
